...
if (!function_exists('ef_category_viewer_row'))
{
    ...
    {
        if (!$cat) return;
        echo '<div class="eventfolio-row ef-viewer">';
        echo '<div class="eventfolio-col ef-col-name">'.esc_html($cat->name).'</div>';
        echo ' <div class="eventfolio-col ef-col-visibility">'.esc_html(ucfirst($cat->visibility ?? '')).'</div>';
        echo '<div class="eventfolio-col ef-col-description">'.esc_html($cat->description).'</div>';
